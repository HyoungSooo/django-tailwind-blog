    ...
    for (let i = 0; i < grid.length - 1; i++) {
        for (let j = 0; j < grid[0].length - 1; j++) {
            ...
            if (grid[i][j + 1].color === northwest_color &&      // Northeast corner
                ...
                grid[i + 1][j + 1].color === northwest_color &&
                northwest_color !== 'white') {  // Southeast corner
                    return true;
                }
        }
    }
...
let k = eval(document.querySelector('.puzzle').innerText)
...
for (let i=0; i < k.length; i ++){
    ...
    for (let j=0; j < k.length; j ++){
      sub_grid.push(new Vertex(i, j, k[i][j]))
    }
    grid.push(sub_grid)
}
...
let container = document.querySelector('.nurikabe-container')
